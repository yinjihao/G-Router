...
import com.example.mysterious.router.components.UriTargetTools;
import com.example.mysterious.router.util.CaseInsensitiveNonNullMap;
...
public class PathHandler extends UriHandler {
    ...
    protected void handleInternal(@NonNull final UriRequest request,
        ...
        if (h != null) {
            h.handleUri(request, new UriCallback() {
                @Override
                public void onNext() {
    ...
    private void handleByDefault(@NonNull UriRequest request, @NonNull UriCallback callback) {
        ...
        if (defaultHandler != null) {
            defaultHandler.handleUri(request, callback);
        } else {
            callback.onNext();
        }
    }
